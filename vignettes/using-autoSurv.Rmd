...
```{r setup}
library(devtools)
load_all("/Users/krithikasuresh/OneDrive - The University of Colorado Denver/sureshk/Projects/Grants/ACSIRG_KS/DiscreteSurvivalPrediction/autoSurv-master")
library(tidyverse)
library(autoSurv)
```
...
The specification of the seed (`seed=1`) allows for these results to be reproduced. 

The following models are fit (`trainModels`): 
* Continuous-time: Cox proportional hazards (`cox`) and Random survival forest (`rsf`)
* Discrete-time models: logistic regression (`glm`), elastic net (`glmnet`), gradient boosting machines (`gbm`), support vector machines (`svm`), and nueral networks (`nnet`)
...
We can obtain the predicted probabilities from a particular individual using the `Score_newdata()` function, and extract the predicted survival probabilities for any of the models that were fit. 

```{r pbc_NewPred, cache=TRUE}
times_pred <- seq(0,3,by=0.01)
pbc.testResults.id <- Score_newdata(pbc.results, newdata=pbc.dat[4,], times=times_pred, timeVar="survTime", statusVar="statusComp")
...
pbc.testid <- ggplot(data = subset(pred_probs_mat ,model%in%c("cox","rsf","glm","nnet")), aes(y = probs, x = times, color = model)) +
  geom_step(size=0.8) +
...
We fit the trained prediction models to a new data set using the `Score_newdata()` function. We specify that the models should be evaluated on the validation data set (`colon.dat_val`) at the time points 1, 2, 3, 4, 5 years (`times=1:5`). We specify the column in the data set for survival time (`timeVar="survTime"`) and event status (`statusVar="status"`) to evaluate the predictive performance of the data set. 

```{r colon_New, cache=TRUE}
colon.testResults <- Score_newdata(colon.results, newdata=colon.dat_val, times=1:5, timeVar="survTime", statusVar="status")
```
